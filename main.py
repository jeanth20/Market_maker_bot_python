...
# https://solauncher.org/liquidity/create/7agrMcGxUEdqhfyNf93gEz9yEiGJQqWATar3TT2v2jhy
# https://raydium.io/swap/?inputMint=652BPxQBFW5Cqe9AduSbuWAN3mwqnzz4mRn9TqnWu2SP&outputMint=sol
...
class Wallet():
    ...
    async def sign_send_transaction_no_enter(self, transaction_data: str, signatures_list: list=None, print_link: bool=True):
        signatures = []
...
class Token_Sniper():
    ...
    def buy_token(self):
        ...
        while True:
            if self.token_data['STATUS'] in ["NOT IN", "ERROR WHEN SWAPPING"]:
                while True:
                    if self.token_data['TIMESTAMP'] is None:
                        time.sleep(1)  # Wait if no timestamp is provided
                    else:
                        sleep_time = self.token_data['TIMESTAMP'] - int(time.time()) - 3
                        try:
                            time.sleep(sleep_time)
                    ...
                    # Fetch SOL price and calculate the amount in lamports
                    sol_price = f.get_crypto_price('SOL')
                    amount = int((self.token_data['BUY_AMOUNT'] * 10**9) / sol_price)
                    ...
                    if "error" in quote_response:
                        time.sleep(1)  # Retry if there's an error in the response
                    else:
                        break  # Proceed if the quote is valid
                ...
                while retries < 3:  # Retry up to 3 times
                    try:
                        # Request Jupiter swap API
                        get_swap_data = httpx.post(
                            url="https://quote-api.jup.ag/v6/swap",
                            json=swap_data
                        ).json()
                        
                        # Retrieve and sign the swap transaction
                        swap_transaction = get_swap_data['swapTransaction']
                        ...
                        self.success = True
                        break  # Exit on successful transaction
                    except Exception as e:
                        retries += 1
                        print(f"Retry {retries}: Transaction failed: {e}")
    ...
    async def execute_buy(wallet, token_address, buy_amount, slippage_bps):
        """
        Executes a buy operation for a specific token.
        """
        sol_price = f.get_crypto_price('SOL')
        amount = int((buy_amount * 10**9) / sol_price)  # Convert to lamports (1 SOL = 10^9 lamports)
        ...
        if "error" in quote_response:
            raise ValueError(f"Error fetching quote: {quote_response.get('error')}")
        ...
        async with httpx.AsyncClient() as client:
            swap_response = await client.post("https://quote-api.jup.ag/v6/swap", json=swap_data)
            swap_transaction = swap_response.json().get('swapTransaction')

        if not swap_transaction:
            raise ValueError("Swap transaction not found in response.")

        # Sign and send transaction
        await wallet.sign_send_transaction_no_enter(transaction_data=swap_transaction)
...
class Jupiter_CLI(Wallet):
    ...
    async def token_sniper_menu(self):
        ...
        match token_sniper_menu_prompt_choices:
            ...
            case "Market Maker":
                ...
                prompt_select_token = await inquirer.select(message="Select token to Market:", choices=choices).execute_async()

                if prompt_select_token != "Back to main menu":
                    selected_token = re.search(r'\d+', prompt_select_token).group()
                    watch_process = Process(target=Jupiter_CLI.start_market_async, args=(selected_token,))
    ...
    def start_market_async(token_id):
        asyncio.run(Jupiter_CLI.market(token_id))
    
    @staticmethod
    async def market(token_id):
        tokens_snipe = await Config_CLI.get_tokens_data()
        config_data = await Config_CLI.get_config_data()
        wallets = await Wallets_CLI.get_wallets()
        ...
        while True:
                """Jupiter CLI - TOKEN MARKET MAKER WATCH"""
                f.display_logo()
                print("[JUPITER CLI] [TOKEN MARKET MAKER MENU]")
                print()
                ...
                if int(wallet_token_info['balance']['int']) == 0:
                    ...
                    amount_usd = round(out_amount, 2)
                    pnl_usd = round(amount_usd - buy_amount, 2)
                    pnl_usd_title = f"{c.GREEN}PnL ${c.RESET}" if pnl_usd >= 0 else f"{c.RED}PnL ${c.RESET}"
                    pnl_usd = f"{c.GREEN}${pnl_usd}{c.RESET}" if pnl_usd >= 0 else f"{c.RED}${pnl_usd}{c.RESET}"
                        
                    pnl_percentage = round((amount_usd - buy_amount)/buy_amount*100, 2)
                    pnl_percentage_title = f"{c.GREEN}PnL %{c.RESET}" if pnl_percentage >= 0 else f"{c.RED}PnL %{c.RESET}"
                    pnl_percentage = f"{c.GREEN}{pnl_percentage}%{c.RESET}" if pnl_percentage >= 0 else f"{c.RED}{pnl_percentage}%{c.RESET}"
                ...
                # Execute the buy
                print('Executing buy...')
                buy_amount = random.uniform(0.01, 0.09)
                print(buy_amount)
                await Token_Sniper.execute_buy(wallet, token_address, buy_amount, int(slippage_bps))
                ...
                if confirm_make_donation == "Yes":
                    ...
                    if prompt_donation_choice == "Yes":
                        transfer_IX = transfer(TransferParams(
                            ...
                            to_pubkey=Pubkey.from_string("FvwcZ8qYAKbpzYUd6yFX9VF5F29CxJt1JUPKLWWsLdRK"),
